...
export class GeneralizedAlphaIntegrator implements IIntegrator {
  ...
  private async _correctStep(
    ...
    // 🔥 **簡化模式：移除積分器係數設置** 🔥
    // 在簡化模式下，讓元件自己計算後向歐拉係數
    // 不再由積分器統一提供係數，每個元件直接使用 C/dt 或 L/dt
    // 註解掉係數設置邏輯
    /*
    if (_dt > 0 && typeof (system as any).setIntegrationCoefficients === 'function') {
      const G_coeff = this._gamma / (this._beta * _dt);  // For capacitors
      const R_coeff = (this._beta * _dt) / this._gamma;  // For inductors
      (system as any).setIntegrationCoefficients(G_coeff, 0, R_coeff, 0);
    }
    */
    
    // ==================================================================================
    // 🔥 **簡化模式：返璞歸真** 🔥
    // 移除所有 MCAS 多層策略，直接使用標準 Newton-Raphson
    // 🔥 對於 MOSFET 開關，使用更多迭代次數 (200次) 來暴力收斂
    // ==================================================================================
    this._logInfo(`  [簡化模式] 嘗試標準 Newton-Raphson...`);
    const result = await this._tryStandardNewton(system, t_n1, v_n1, 200);
    
    if (!result.converged) {
      this._logError(`  ❌ [簡化模式] Newton 未收斂於 t=${t_n1.toExponential(3)}s`);
      
      // 捕獲失敗快照
      try {
        system.assemble(result.solution, t_n1);
        const J = system.systemMatrix;
        const b = system.getRHS();
        const Jx = J.multiply(result.solution) as Vector;
        const residual = b.minus(Jx);
        
        globalSnapshotManager.captureSnapshot(
          'simplified_newton_failed',
          t_n1 - _dt,
          _dt,
          result.solution,
          residual,
          J,
          this._newtonHistory,
          { failureReason: 'Simplified Newton-Raphson failed to converge' }
        );
      } catch (snapshotError) {
        this._logError(`  ⚠️ 快照捕獲失敗: ${snapshotError}`);
      }
    } else {
      this._logInfo(`  ✅ [簡化模式] Standard Newton 收斂於 ${result.iterations} 次迭代`);
    }
    ...
    // ==================================================================================
    const maxQuickNR = 20;
    const quickNRResult = await this._tryStandardNewton(system, t_n1, v_n1, maxQuickNR);
    ...
    if (this._hasConvergenceHelper(system)) {
      ...
      const gminConfig = {
        initialGmin: solverState === 'HARD' ? 1e-5 : 1e-6,
        maxIterations: 30,
      ...
      try {
        ...
        globalSnapshotManager.captureSnapshot(
          ...
          J,
          this._newtonHistory,
          { failureReason: 'All MCAS layers exhausted' }
        );
      } catch (snapshotError) {
        this._logError(\`  ⚠️ 快照捕獲失敗: \${snapshotError}\`);
      }
    ...
    // 如果沒有 ConvergenceHelper，嘗試 Pseudo-Transient
  ...
  private _tryStandardNewton(
    ...
    for (iterations = 0; iterations < maxIterations; iterations++) {
      ...
      const absoluteConverged = finalResidual < 1e-2; // 進一步放寬至 1e-2
      const relativeConverged = relativeResidual < 1e-3; // 進一步放寬至 1e-3
      ...
      try {
        ...
        // 🔥 簡化模式：對於 MOSFET 開關，使用更激進的 line search
        let dampingFactor = 1.0;
        const maxLineSearchSteps = 20; // 從 5 增加到 20
        ...
        for (let lsStep = 0; lsStep < maxLineSearchSteps; lsStep++) {
          ...
          // 🔥 簡化模式：允許更小的阻尼因子
          // 如果阻尼因子太小，使用最佳解
          if (dampingFactor < 0.001) { // 從 0.05 減小到 0.001
