 ...
 *
 * 🚀 新增積分器解耦設計：
 * G_coeff 和 I_coeff 由積分器提供，使無源元件（電容、電感）
 * 能夠自動適應積分器的精度，無需硬編碼 Backward Euler 公式
 */
export interface AssemblyContext {
  ...
  // --- 🚀 新增：積分器提供的係數 (用於解耦無源元件) ---

  /**
   * 電導係數 (G_coeff)
   * 用於計算電容的等效電導：G_eq = C * G_coeff
   * 對於 Backward Euler: G_coeff = 1/dt
   * 對於 Generalized-α: G_coeff ≈ gamma/(beta*dt)
   */
  readonly G_coeff?: number;

  /**
   * 歷史電流係數 (I_coeff)
   * 用於計算電容的等效電流源：I_eq = C * I_coeff
   * I_coeff 包含了與歷史電壓、速度、加速度相關的項
   * 對於 Backward Euler: I_coeff = V_prev/dt
   * 對於 Generalized-α: I_coeff 是更複雜的歷史狀態組合
   */
  readonly I_coeff?: number;

  /**
   * 電阻係數 (R_coeff) - 用於電感
   * 用於計算電感的等效電阻：R_eq = L * R_coeff
   * 對於 Backward Euler: R_coeff = 1/dt
   * 對於 Generalized-α: R_coeff ≈ gamma/(beta*dt)
   */
  readonly R_coeff?: number;

  /**
   * 歷史電壓係數 (V_coeff) - 用於電感
   * 用於計算電感的等效電壓源：V_eq = L * V_coeff
   * V_coeff 包含了與歷史電流、速度相關的項
   */
  readonly V_coeff?: number;
}
...
export interface ComponentInterface {
   ...
   * @returns 断点时间数组 (升序排列,且在 [startTime, endTime] 范围内)
   */
  ...
  /**
   * ⚡ 计算通过组件的电流 (用于 KCL 残差计算)
   *
   * 根据给定的电压向量计算组件的实际电流
   * 对于线性组件，使用 Ohm 定律或伴侣模型
   * 对于非线性组件，必须包括指数项 (例如 Shockley 方程)
   *
   * @param voltages - 系统的完整电压向量
   * @param context - 组装上下文 (可选，用于获取额外变量)
   * @returns 组件电流 (正值表示从正节点流向负节点)
   */
