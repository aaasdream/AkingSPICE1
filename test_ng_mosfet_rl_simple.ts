...
async function testNgMosfetRLCircuit() {
  ...
  try {
    ...
    if (result.success && result.waveformData) {
      ...
      for (let i = 0; i < timePoints.length; i++) {
        if (timePoints[i] >= period1End && idx1 === 0) idx1 = i;
        if (timePoints[i] >= period2End && idx2 === 0) idx2 = i;
      }

      // 計算電感電流 (透過電壓推算)
      const calcInductorCurrent = (idx: number) => {
        const vdrain = vDrain?.[idx] || 0;
        const vsource = 0; // 接地
      ...
      const vgate1 = vGate?.[idx1] || 0;
      const vdrain1 = vDrain?.[idx1] || 0;
      ...
      const vgate2 = vGate?.[idx2] || 0;
      const vdrain2 = vDrain?.[idx2] || 0;
      ...
      for (let i = 0; i < timePoints.length; i++) {
        csv += `${timePoints[i]},${vGate?.[i] || 0},${vDrain?.[i] || 0},${vRL?.[i] || 0}\n`;
      }
