...
console.log('🎯 測試場景: MOSFET 在 t=0 時從 OFF → ON');
...
engine.runSimulation()
  .then(result => {
    ...
    console.log(`   平均步長時間: ${(result.averageStepTime * 1e6).toFixed(2)}µs`);
    ...
    if (result && result.waveformData) {
      ...
      if (deviceCurrents && deviceCurrents instanceof Map && deviceCurrents.has('L1')) {
        ...
        if (iL && iL.length > 0) {
          ...
          console.log(`   峰值: ${Math.max(...iL).toExponential(3)} A`);
          console.log(`   終值: ${iL[n-1].toExponential(3)} A`);
          
          // 理論值: L di/dt = V → di/dt = V/L
          // V ≈ VDD (忽略電阻壓降)
          // i(t) ≈ (V/L) * t = (12/100µH) * t = 1.2e5 * t
          // t=10µs: i ≈ 1.2A
          const theoreticalCurrent = (VDD / L_LOAD) * T_SIM;
          console.log(`   理論終值: ${theoreticalCurrent.toExponential(3)} A`);
          
          const error = Math.abs((iL[n-1] - theoreticalCurrent) / theoreticalCurrent) * 100;
          console.log(`   誤差: ${error.toFixed(1)}%`);
          
          if (error < 20) {
            console.log(`   ✅ 誤差在可接受範圍內 (<20%)`);
          } else {
            console.log(`   ⚠️  誤差較大 (>20%)`);
          }
